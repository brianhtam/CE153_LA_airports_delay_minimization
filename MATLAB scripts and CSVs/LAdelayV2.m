...
holdlim=0;  %set max number of holds
ERdelaylim=0;  %set max amount of en route delay
DelayLim=LM/60;% 10/60;   %delay limit for any aircraft in hours

lb = zeros(length(c),1); %lower bound on all variables
lb(1:N)= ARRn(start,18);
ub = [(ARRn(start+sample-1,18)+DelayLim)*ones(N,1); ones(N*R+N*(N-1)/2,1);holdlim(ones(N,1));ERdelaylim(ones(N,1))]; %upper bound on all variables
offLimits = (7-OL):7;%Set off limit runways
for i = offLimits
    ub(N+i:R:N+R*(N-1)+i) = 0;%impose upper limit constraints on off limits runways
end
...
%% Clear Source and Initialize Arrays
clear ARR*
...
if isempty(Sched)
    ...
    timespan=ub(1)-lb(1);
    AveTimespan=(timespan*(NG-1)+AveTimespan)/NG;
end
